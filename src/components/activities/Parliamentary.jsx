...
function LegislationItem({ item, index, inView }) {
  ...
  return (
    <motion.div
      ...
      className="border-b border-[#e8e7e6]/[0.12] last:border-b-0"
    >
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-4 md:gap-6 py-5 text-left focus:outline-none group"
      >
        <span className="shrink-0 text-[0.6rem] font-black tracking-[0.15em] text-[#e8e7e6]/20 w-5"
        ...
        <span className="flex-1 text-[1rem] md:text-[1.1rem] font-black text-[#e8e7e6]/85 group-hover:text-[#e8e7e6] transition-colors duration-200 leading-snug"
          style={{ fontFamily: "'Playfair Display', serif" }}>
          {item.title}
        </span>

        <span className="hidden md:block shrink-0 text-[0.6rem] font-bold tracking-[0.1em] uppercase text-[#e8e7e6]/25 mr-3"
          style={{ fontFamily: "Raleway, sans-serif" }}>
          {item.house}
        </span>

        <div className="shrink-0 w-8 h-8 border border-[#e8e7e6]/10 group-hover:border-[#fc8814]/40 flex items-center justify-center transition-colors duration-200"
          style={{ color: item.color }}>
          <Icon size={14} strokeWidth={1.6} />
        </div>

        <motion.div animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.3 }} className="shrink-0">
          <ChevronDown size={15} className="text-[#e8e7e6]/25" />
        </motion.div>
      </button>
          ...
          >
            <div className="pl-[4.5rem] pr-4 pb-5 flex gap-4">
              ...
              <div>
                ...
                <p className="text-[0.93rem] text-[#e8e7e6]/50 leading-[1.85] italic"
...
export default function ParliamentaryWork() {
  ...
  return (
    <section id="parliamentary-work" className="relative bg-[#1a1a1a] overflow-hidden py-20 md:py-28">
      ...
      <div className="relative z-10 max-w-[1280px] mx-auto px-5 md:px-10 lg:px-16">
        ...
        <div ref={headerRef} className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-20 items-end mb-14 md:mb-16">
          <div>
            ...
            <motion.h2 variants={fadeUp(0.08)} initial="hidden" animate={headerInView ? "show" : "hidden"}
              className="text-[2.8rem] md:text-[3.8rem] lg:text-[4.2rem] font-black text-[#e8e7e6] leading-[0.93]"
              style={{ fontFamily: "'Playfair Display', serif" }}>
              Parliamentary<br /><em className="text-[#fc8814] not-italic italic">Work</em>
            </motion.h2>
          </div>
          <div>
            ...
            <motion.p variants={fadeUp(0.2)} initial="hidden" animate={headerInView ? "show" : "hidden"}
              className="text-[1.05rem] text-[#e8e7e6]/40 leading-[1.9] italic"
        ...
        {/* ── STATS ── */}
        <div ref={statsRef} className="grid grid-cols-2 lg:grid-cols-4 gap-0 mb-16 md:mb-20 border border-[#e8e7e6]/[0.08]">
          {stats.map((s, i) => {
            const Icon = s.icon;
            return (
              <motion.div
                ...
                className={`group relative flex flex-col gap-2 p-6 md:p-8
                  bg-[#252525]/50 hover:bg-[#252525] transition-colors duration-300 overflow-hidden
                  ${i < 3 ? "border-r border-[#e8e7e6]/[0.07]" : ""}
                  ${i < 2 ? "border-b border-[#e8e7e6]/[0.07] lg:border-b-0" : ""}`}
              >
                <Icon size={20} className="text-[#fc8814] mb-1" strokeWidth={1.5} />
                <span className="text-[2.8rem] md:text-[3.2rem] font-black text-[#fc8814] leading-none"
                  style={{ fontFamily: "'Playfair Display', serif" }}>{s.num}</span>
                <span className="text-[0.6rem] font-bold tracking-[0.18em] uppercase text-[#e8e7e6]/30"
        ...
        <div ref={listRef}>
          <motion.div variants={fadeUp(0)} initial="hidden" animate={listInView ? "show" : "hidden"}
            ...
            <span className="text-[0.6rem] font-black tracking-[0.3em] uppercase text-[#e8e7e6]/20"
              style={{ fontFamily: "Raleway, sans-serif" }}>Key Legislation — Click to expand</span>
            <div className="flex-1 h-[1px] bg-[#e8e7e6]/[0.07]" />
          </motion.div>

          <div className="border border-[#e8e7e6]/[0.07] bg-[#252525]/40 px-4 md:px-8">
